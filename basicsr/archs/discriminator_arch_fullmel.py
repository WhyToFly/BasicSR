...
class VGGStyleDiscriminator128(nn.Module):
    ...
    def forward(self, x):
        #assert x.size(2) == 512 and x.size(3) == 128, (f'Input spatial size must be 512x128, but received {x.size()}.')

        feat = self.lrelu(self.conv0_0(x))
        feat = self.lrelu(self.bn0_1(self.conv0_1(feat)))  # output spatial size: (64, 64) - actually 256x64
        

        feat = self.lrelu(self.bn1_0(self.conv1_0(feat)))
        feat = self.lrelu(self.bn1_1(self.conv1_1(feat)))  # output spatial size: (32, 32) - actually 128x32
        

        feat = self.lrelu(self.bn2_0(self.conv2_0(feat)))
        feat = self.lrelu(self.bn2_1(self.conv2_1(feat)))  # output spatial size: (16, 16) - actually 64x16
        

        feat = self.lrelu(self.bn3_0(self.conv3_0(feat)))
        feat = self.lrelu(self.bn3_1(self.conv3_1(feat)))  # output spatial size: (8, 8) - actually 32x8
        

        feat = self.lrelu(self.bn4_0(self.conv4_0(feat)))
        feat = self.lrelu(self.bn4_1(self.conv4_1(feat)))  # output spatial size: (4, 4) - actually 16x4
                
        # correct size by max pooling in height
        feat = nn.functional.max_pool2d(feat, kernel_size=[4,1], padding=[1,0])
